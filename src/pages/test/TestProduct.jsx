// Componenta TestProduct
export const TestProduct = ({
  id,
  title,
  price,
  description,
  category,
  image,
  addProductToCart,
}) => {
  return (
    <>
      <div className="w-full sm:w-60 bg-white rounded-xl shadow-lg p-5 hover:shadow-2xl transform hover:scale-105 transition-all duration-300">
        {/* Imaginea produsului */}
        <img
          src={image}
          alt={title}
          className="h-40 w-full object-cover rounded-md mb-4"
        />

        {/* Titlul produsului */}
        <h3 className="font-semibold mb-2">{title}</h3>

        {/* Categoria */}
        <p className="text-sm text-gray-600 mb-2">Category: {category}</p>

        {/* Descriere */}
        <p className="text-sm text-gray-800 mb-2 truncate">{description}</p>

        {/* Prețul */}
        <p className="text-lg font-bold text-green-500 mb-4">${price}</p>
        <button onClick={addProductToCart}>
          {" "}
          <svg fill="currentColor" viewBox="0 0 16 16" height="2em" width="2em">
            <path d="M11.354 6.354a.5.5 0 00-.708-.708L8 8.293 6.854 7.146a.5.5 0 10-.708.708l1.5 1.5a.5.5 0 00.708 0l3-3z" />
            <path d="M.5 1a.5.5 0 000 1h1.11l.401 1.607 1.498 7.985A.5.5 0 004 12h1a2 2 0 100 4 2 2 0 000-4h7a2 2 0 100 4 2 2 0 000-4h1a.5.5 0 00.491-.408l1.5-8A.5.5 0 0014.5 3H2.89l-.405-1.621A.5.5 0 002 1H.5zm3.915 10L3.102 4h10.796l-1.313 7h-8.17zM6 14a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z" />
          </svg>
        </button>
      </div>
    </>
  );
};
